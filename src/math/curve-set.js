Object.assign(vs, (function () {
    ...
    var CurveSet = function () {
        ...
        this.curves = []
        this._type = vs.CURVE_SMOOTHSTEP

        if (arguments.length > 1) {
            for (i = 0; i < arguments.length; i++) {
                this.curves.push(new vs.Curve(arguments[i]))
            }
        } else {
            if (arguments.length === 0) {
                this.curves.push(new vs.Curve())
            } else {
                var arg = arguments[0]
                if (pc.type(arg) === 'number') {
                    ...
                    for (i = 0; i < arg.length; i++) {
                        this.curves.push(new vs.Curve(arg[i]))
                    }
                }
            }
        }
    ...
    Object.assign(CurveSet.prototype, {
        ...
        /**
         * @function
         * @name vs.CurveSet#clone
         * @description Returns a clone of the specified curve set object.
         * @returns {vs.CurveSet} A clone of the specified curve set
         */
        clone: function () {
            var result = new vs.CurveSet()
        ...
        quantize: function (precision) {
            ...
            for (var c = 0; c < numCurves; c++) {
                var ev = new vs.CurveEvaluator(this.curves[c])
         ...
         * @private
         * @function
         * @name vs.CurveSet#quantizeClamped
    ...
    /**
     * @name vs.CurveSet#type
     * @type Number
     * @description The interpolation scheme applied to all curves in the curve set. Can be:
     * <ul>
     *     <li>{@link vs.CURVE_LINEAR}</li>
     *     <li>{@link vs.CURVE_SMOOTHSTEP}</li>
     *     <li>{@link vs.CURVE_SPLINE}</li>
     *     <li>{@link vs.CURVE_STEP}</li>
     * </ul>
     * Defaults to {@link vs.CURVE_SMOOTHSTEP};
     */
